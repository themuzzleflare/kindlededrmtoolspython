...
from dedrmtools.mobidedrm.pukallcipher import PukallCipher
...
class MobiBook(Book):
    ...
    def get_size_of_trailing_data_entries(ptr, size, flags):
        ...
        while testflags:
            if testflags & 1:
                num += MobiBook.get_size_of_trailing_data_entry(ptr, size - num)
            testflags >>= 1
            ...
            if (len(exth) >= 12) and (exth[:4] == b'EXTH'):
                ...
                for i in range(nitems):
                    ...
                    if type == 401 and size == 9:
                        ...
                        self.patch_section(0, b'\0' * 8, 16 + self.mobi_length + pos + 8)
                    elif type == 208:
                        # remove watermark (atv:kin: stuff)
                        self.patch_section(0, b'\0' * (size - 8), 16 + self.mobi_length + pos + 8)
    ...
    def process_book(self, pidlist):
        ...
        for pid in pidlist:
            ...
            if len(pid) == 10:
                if checksum_pid(pid[0:-2]) != pid:
                    print("Warning: PID {0} has incorrect checksum, should have been {1}".format(pid, checksum_pid(
                        pid[0:-2])))
        ...
        if self.crypto_type == 1:
            ...
            found_key, pid = self.parse_drm(self.sect[drm_ptr:drm_ptr + drm_size], drm_count, goodpids)
            if not found_key:
        ...
        mobidata_list = [self.data_file[:self.sections[1][0]]]
        for i in range(1, self.records + 1):
            data = self.load_section(i)
            extra_size = self.get_size_of_trailing_data_entries(data, len(data), self.extra_data_flags)
            if i % 100 == 0:
                print(".", end=' ')
            ...
            mobidata_list.append(decoded_data)
            if extra_size > 0:
...
def get_unencrypted_book(infile, pidlist):
    if not os.path.isfile(infile):
        raise DrmException("Input File Not Found.")
    book = MobiBook(infile)
    book.process_book(pidlist)
